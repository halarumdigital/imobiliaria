...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // API settings for clients (Client role)
  ...
  app.get("/api/client/test-api-settings", authenticate, requireClient, async (req: AuthRequest, res) => {
    try {
      ...
      try {
        const searchParams = {
          fields: ["Codigo", "Categoria", "BairroComercial", "Cidade"],
          paginacao: { pagina: "1", quantidade: "1" }
        };
        
        const baseUrl = `${settings.apiUrl}/imoveis/listar`;
        const pesquisaParam = encodeURIComponent(JSON.stringify(searchParams));
        const apiUrl = `${baseUrl}?key=${settings.apiToken}&pesquisa=${pesquisaParam}&showtotal=1`;

...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // API settings for clients to manage VistaHost integration
  app.get("/api/client/api-settings", authenticate, requireClient, async (req: AuthRequest, res) => {
    try {
      console.log("🔍 [CLIENT-API-SETTINGS] Request received");
      console.log("🔍 [CLIENT-API-SETTINGS] User:", req.user);
      console.log("🔍 [CLIENT-API-SETTINGS] Company ID:", req.user?.companyId);
      
      if (!req.user?.companyId) {
        console.log("❌ [CLIENT-API-SETTINGS] No company ID found");
        return res.status(404).json({ error: "Empresa não encontrada" });
      }
      
      const settings = await storage.getApiSettings(req.user.companyId);
      console.log("🔍 [CLIENT-API-SETTINGS] Settings found:", settings ? "Yes" : "No");
      console.log("🔍 [CLIENT-API-SETTINGS] Settings data:", settings);
  ...
  app.put("/api/client/api-settings", authenticate, requireClient, async (req: AuthRequest, res) => {
    try {
      ...
      const { apiUrl, apiToken, isActive } = req.body;
      
      if (!apiUrl || !apiToken) {
        return res.status(400).json({ error: "URL da API e Token são obrigatórios" });
      }
  ...
  app.get("/api/client/test-api-settings", authenticate, requireClient, async (req: AuthRequest, res) => {
    try {
      ...
      const apiSettings = await storage.getApiSettings(req.user.companyId);
      
      if (!apiSettings?.apiUrl || !apiSettings?.apiToken) {
        return res.json({
          success: false,
          message: "Configurações da API não encontradas. Configure a URL e Token da API primeiro.",
          details: {
            hasUrl: !!apiSettings?.apiUrl,
            hasToken: !!apiSettings?.apiToken
          }
      ...
      console.log("🔍 [CLIENT-TEST] Testing API connection...");

      const response = await fetch(apiUrl, {
        headers: { 'Accept': 'application/json' }
      });

      console.log("🔍 [CLIENT-TEST] Response status:", response.status, response.statusText);

      if (!response.ok) {
        const errorText = await response.text();
        console.log("🔍 [CLIENT-TEST] Error response:", errorText);
        return res.json({ 
          success: false,
          message: `Erro na conexão com a API: ${response.status} - ${response.statusText}`,
          details: {
            status: response.status,
            statusText: response.statusText,
            body: errorText
      ...
      const data = await response.json();
      console.log("🔍 [CLIENT-TEST] Response structure:", {
        type: typeof data,
        isArray: Array.isArray(data),
        length: Array.isArray(data) ? data.length : 'N/A'
      });
      ...
      return res.json({
        success: true,
        message: `Conexão com a API estabelecida com sucesso! ${Array.isArray(data) ? `Encontrados ${data.length} imóveis de teste.` : 'API respondeu corretamente.'}`,
        details: {
          status: response.status,
          statusText: response.statusText,
          dataType: typeof data,
      ...
      res.status(500).json({ 
        success: false,
        message: "Erro interno ao testar a conexão com a API",
        details: {
          error: error instanceof Error ? error.message : 'Unknown error'
        }
      });
